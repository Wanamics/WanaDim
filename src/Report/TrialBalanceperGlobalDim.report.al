...
// from NA Localization report 10023
{
    ...
    dataset
    {
        dataitem("Dimension Value"; "Dimension Value")
        {
            ...
            dataitem("G/L Account"; "G/L Account")
            {
                ...
                column(DimensionValueName; /*"Dimension Value".Name + ' ' + Text014*/ StrSubstNo(wanTitle, "Dimension Value".Name))
                {
                }
    ...
    begin
        ...
        */
        ...
        if ColumnFlags[7] then begin                              // YTD
            PeriodText := /*'As of '*/ wanAsOf + ' ' + Format(ToDate, 0, 4);
            if ColumnFlags[9] then begin
                if Show = Show::Budget then                           // Budget
                    PeriodText := /*'Actual vs Budget a'*/ wanActualVsBudgetA + DelStr(PeriodText, 1, 1)
                else                                                  // Prior YTD
                    PeriodText := PeriodText + /*' and '*/ ' ' + wanAnd + ' ' + Format(Date2DMY(PriorToDate, 3));
            end;
        end else
            if ColumnFlags[9] then begin
                if Show = Show::Budget then                             // Budget
                    PeriodText := /*'Budget as of '*/ wanBudgetAsOf + ' ' + Format(ToDate, 0, 4)
                else                                                    // Prior YTD
                    PeriodText := /*'As of '*/ wanAsOf + ' ' + Format(PriorToDate, 0, 4);
            end else
                if ColumnFlags[1] then begin                     // PTD
                    PeriodText := /*'Changes '*/ wanChanges + ' ';
                    if ColumnFlags[3] and (Show = Show::Budget) then        // Budget
                        PeriodText := PeriodText + /*'and Budgeted Changes '*/ wanAndBudgetedChanges;
                    PeriodText := PeriodText + /*'from '*/ wanFrom + ' '
                      + Format(FromDate, 0, 4)
                      + ' to '
                      + Format(ToDate, 0, 4);
                    if ColumnFlags[3] and (Show = Show::"Last Year") then   // Prior PTD
                        PeriodText := PeriodText + /*' and from '*/ ' ' + wanAndFrom + ' '
                    ...
                    if ColumnFlags[3] then begin                     // PTD
                        if Show = Show::Budget then                           // Budget
                            PeriodText := /*'Budgeted '*/ wanBudgeted + ' '
                        else                                                    // Prior PTD
                            PeriodText := '';
                        PeriodText := PeriodText + /*'Changes from '*/ wanChangesFrom + ' ';
                        if Show = Show::Budget then                           // Budget
    ...
    var
        ...
        wanTitle: Label '%1 Trial Balance';
        wanAsOf: Label 'as of';
        wanActualVsBudgetA: Label 'Actual vs Budget a';
        wanAnd: Label 'and';
        wanBudgetAsOf: Label 'Budget as of';
        wanChanges: Label 'Changes';
        wanAndBudgetedChanges: Label 'and Budget Changes';
        wanAndFrom: Label 'and from';
        wanFrom: Label 'from';
        wanBudgeted: Label 'Budgeted';
        wanChangesFrom: Label 'Changes from';
        wanTo: Label 'to';
