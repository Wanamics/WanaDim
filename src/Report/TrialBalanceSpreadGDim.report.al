...
// from NA Localization report 10025
{
        ...
        */
        ...
        case AmountType of
            /*
                ...
                */
            AmountType::"Actual Change":
                PeriodText := wanChangesFrom + ' ' + Format(FromDate, 0, 4) + wanTo + Format(ToDate, 0, 4);
            AmountType::"Budget Change":
                PeriodText := wanBudgetedChangesFrom + ' ' + Format(FromDate, 0, 4) + wanTo + Format(ToDate, 0, 4);
            AmountType::"Last Year Change":
                PeriodText := wanChangesFrom + ' ' + Format(PriorFromDate, 0, 4) + wanTo + Format(PriorToDate, 0, 4);
            AmountType::"Actual Balance":
                PeriodText := wanAsOf + ' ' + Format(ToDate, 0, 4);
            AmountType::"Budget Balance":
                PeriodText := wanBudgetAsOf + ' ' + Format(ToDate, 0, 4);
            AmountType::"Last Year Balance":
                PeriodText := wanAsOf + ' ' + Format(PriorToDate, 0, 4);
        end;
        if UseAddRptCurr then begin
            GLSetup.Get();
    ...
    begin
        ...
        ExcelBuf.WriteSheet(Dimension.Name + ' ' + Text102, CompanyName, UserId);
        ExcelBuf.CloseBook;
        ExcelBuf.SetFriendlyFilename(
            FileManagement.GetSafeFileName(
                Dimension.Name + ' ' +
                Format("G/L Account".GetRangeMin("Date Filter"), 0, 9) + ' ' +
                Format("G/L Account".GetRangeMax("Date Filter"), 0, 9)));
        ExcelBuf.OpenExcel;
    end;
#endif
}
